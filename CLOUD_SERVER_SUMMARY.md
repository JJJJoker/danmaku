...
将原有的P2P架构改造为客户端-服务器架构,通过云服务器(REDACTED_SERVER_IP)维护房间信息和转发消息,解决Host离线导致房间不可用的问题。
...
scp server.tar.gz root@REDACTED_SERVER_IP:/opt/danmaku-server/
...
2. **从本地连接测试**:
   - 修改 `SERVER_URL` 为 `ws://REDACTED_SERVER_IP:8080`
   - 运行客户端
   - 验证连接和功能
...
Client A --\
             \
Client B ----> 云服务器 (REDACTED_SERVER_IP)
             /    |-- 房间管理
Client C --/     |-- 消息转发
                  |-- 心跳保活
                  |-- 数据存储
...
当前配置: `ws://REDACTED_SERVER_IP:8080`
