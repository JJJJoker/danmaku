...
服务器 IP `116.62.47.225` 与自建 TURN 服务器的真实凭据（`root/tth012...`，本报告不再复现全文）自根提交起就硬编码在源码和大量文档中，污染了全部提交历史。整改思路：客户端默认服务器地址本质是公开信息（必然打进发布安装包），所以方案不是"藏地址"而是**让源码与仓库不含任何真实基础设施信息**——地址改为「设置自定义 > CI 构建期注入 > 空」三级解析；泄露的 TURN 凭据属于真实机密，必须在服务器侧轮换/停用（历史重写不能撤回已泄露的值）。P2P 模式（TURN 的唯一使用方）本身是遗留代码（默认走服务器中继、UI 无入口），借此机会彻底移除。
...
116.62.47.225==>REDACTED_SERVER_IP
...
- 验证：全新 clone 上 `git log -S '116.62.47.225' --all` 与凭据同款搜索均应为空
