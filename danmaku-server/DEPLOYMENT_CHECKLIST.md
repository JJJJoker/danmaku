...
- [ ] SSH可以连接到服务器 (ssh root@REDACTED_SERVER_IP)
...
服务器地址: ws://REDACTED_SERVER_IP:8080
服务状态: online
```
...
**从本地测试网络连通性**:
```powershell
telnet REDACTED_SERVER_IP 8080
```
...
下载到本地:
```powershell
scp root@REDACTED_SERVER_IP:/opt/danmaku-server-backup-20260705.tar.gz .
```
