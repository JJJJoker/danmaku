...
1. **使用scp上传文件**:
   ...
   scp danmaku-server.tar.gz root@116.62.47.225:/opt/danmaku-server/
   ```
...
```bash
# 在本地使用scp命令
scp -r danmaku-server/* root@116.62.47.225:/opt/danmaku-server/
```
...
# 下载到本地
scp root@116.62.47.225:/opt/danmaku-backup-*.tar.gz .
```
...
```bash
# 上传备份
scp danmaku-backup-*.tar.gz root@116.62.47.225:/tmp/
